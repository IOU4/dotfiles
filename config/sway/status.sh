...
echo '[]'
js='"name": "time", "border": "ffffff" ,"separator": false, "full_text": '
...
do
    ...
    vol="\"墳  $(amixer -D pulse get Master | awk -F 'Left:|[][]' 'BEGIN {RS=""}{ print $3 }')\""

    ram="\"﬙  $(free --mega | awk '/Mem/{printf("%.1f%"), $3/$2*100}')\""

    cpu="\"龍  $(grep 'cpu ' /proc/stat | awk '{printf("%.1f%"),($2+$4)*100/($2+$4+$5)}')\""

    echo ",[{$js $date},{$js $time},{$js $bat},{$js $light},{$js $vol},{$js $ram},{$js $cpu}]"
	sleep 5
done
